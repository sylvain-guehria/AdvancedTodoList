  ...
  <div>
    <md-table class="table-custom no-border">
      <md-table-row v-if="item.subtasks && item.subtasks.length">
        ...
        <md-table-head width="150px">Details</md-table-head>
      ...
      <md-table-row v-for="(subtask, index) in item.subtasks" :key="index">
        ...
        <md-table-head width="150px">{{ subtask.label }}</md-table-head>
        <md-table-head width="150px">{{ subtask.detail }}</md-table-head>
        <md-table-head width="50px">{{ subtask.deadline }}</md-table-head>
        <md-table-head width="50px">{{ subtask.importance }}</md-table-head>

        <md-table-head 
        width="50px"
          ><feather type="check" class="hover-click" v-if="subtask.isdone"  @click="setSubTaskState(subtask.key, item.key, false)"></feather>
          <feather type="x" class="hover-click" v-if="!subtask.isdone"  @click="setSubTaskState(subtask.key, item.key, true)"></feather>
        </md-table-head>
      ...
      <md-table-row >
        <md-table-head colspan="6">
          <feather type="plus-circle" class="hover-click" @click="addSubtaskModal(item.key)"></feather
        ></md-table-head>
      </md-table-row>
    </md-table>
